...
export const AdminUsers: React.FC = () => {
  ...
  const handlePasswordReset = (e: React.FormEvent) => {
    ...
    if (resetUserId && newResetPassword) {
      updateUserPassword(resetUserId, newResetPassword);
      setResetUserId(null);
      setNewResetPassword('');
          ...
          <CardContent>
            <form onSubmit={handleAdd} className="space-y-4">
              ...
              <div>
                ...
                <input type="password" required value={password} onChange={e => setPassword(e.target.value)} className="w-full px-3 py-2 border rounded-lg bg-white dark:bg-gray-700 dark:border-gray-600" />
              </div>
              <div>
      ...
      >
        <form onSubmit={handlePasswordReset} className="space-y-4">
           <p className="text-sm text-gray-600 dark:text-gray-400">
             Insira a nova senha para: <strong>{users.find(u => u.id === resetUserId)?.name}</strong>
           </p>
           <div>
